WITH strk_tx_summary AS (
    SELECT block_date, block_time, block_number, hash, block_l1_da_mode, block_l1_data_gas_price_in_fri, block_l1_data_gas_price_in_wei,
           ...
           (p.price / eth_price.price) * actual_fee_amount / pow(10, 18) as fee_eth,
    ...
    ON p.contract_address IS NULL
       ...
       AND (CASE 
            ...
            END) = p.symbol
    LEFT JOIN prices.usd_daily eth_price
    ON  eth_price.contract_address IS NULL
    AND eth_price.symbol = 'ETH'
    AND eth_price.day = t.block_date
    WHERE date_trunc('month',block_date) >= date_trunc('month',now() - interval '3' month)
